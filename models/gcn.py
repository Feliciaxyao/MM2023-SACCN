...
class GraphAttention(nn.Module):
    ...
    def forward(self, input, adj):
        h = torch.bmm(input, self.W)
        N = h.size()[0]

        f_1 = torch.matmul(h, self.a1)
        f_2 = torch.matmul(h, self.a2)
        e = self.leakyrelu(f_1 + f_2.transpose(0, 1))
        #e = F.relu(f_1 + f_2.transpose(0, 1))
...
class GCN(nn.Module):
    ...
    def __init__(
        ...
        # print(hidden_size)
        self.bn_feat = nn.BatchNorm1d(input_size)
        self.layers.append(GraphConvolution(input_size, hidden_size))
        self.bns_conv = torch.nn.ModuleList()
        self.bns_conv.append(nn.BatchNorm1d(input_size))
        ...
        self.edge_dropout= nn.Dropout(p=0.1)
        self.node_dropout= nn.Dropout(p=0.1)
        self.use_norm = True

    def edge_drop(self,adj):
        adj_=self.edge_dropout(adj)
        return adj_

    def node_drop(self,x):
        x_=self.node_dropout(x)
        return x_
    ...
    def forward(self, x, adj):
        # print(x.shape)
        # user batch Norm，使用之前需要做transpose
        # if(self.use_norm):
         #    x=self.norm(x)
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Edge Drop operation
            # 
            # x_ = F.relu(layer(x,adj_))
            # Node drop operation
            # adj=self.edge_drop(adj)

             if(i==0):
                adj_=self.edge_drop(adj)
                x_ = F.relu(layer(x,adj_))
             else:
                x=self.node_drop(x)
                x_ = F.relu(layer(x,adj))
