...
class ViolinDataset(Dataset):
    def __init__(self, opt, mode='train'):
        ...
        if 'vid' in self.input_streams:
            ...
            with h5py.File(os.path.join(opt.feat_dir, 'all_c3d_fc6_features.h5' ), 'r') as fin:  #if opt.feat == 'resnet' else 'all_res101_pool5_feat.h5'
                 for clip_id in tqdm(fin.keys()):
                    if clip_id in clip_set:
                        if opt.frame == '':
                            self.vid_feat[clip_id] = torch.Tensor(np.array(fin[clip_id+'/{}_features'.format(opt.feat[0])]))
                        else:
        ...
        for clip in tqdm(self.clip_info):
            ...
            clip['answer'] = []
            clip['padded_statement'] = []
            clip['padded_sub'] = ' '.join([clean_str(ss[0]).lower() for ss in clip['sub']])
            # get statement
            # clip['padded_statement'] = [[self.tokenize_and_pad(clean_str(pair[i]).lower()) for i in range(2)] for pair in clip['statement']]
            clip['padded_statement'] = [[clean_str(pair[i]).lower() for i in range(2)] for pair in clip['statement']]
            # for pair in clip['statement']:
            #     idx = range(2)
            ...
            #     pair = [tmp1, tmp2]
            #     if i1 == 0:
            #         clip['answer'].append(0)
            #     else:
            #         clip['answer'].append(1)
                
            #     tmp = []
            #     for i in range(2):
            #         tmp.append(clean_str(pair[i]).lower())
            #     clip['padded_statement'].append(tmp)
    ...
    def __getitem__(self, idx):
        ...
        return clip['file'], vid_feat, sub_input, state_input, modality_text, modality_video

    def get_state_pair(self, idx):
        clip = self.clip_info[int(idx/3)]
        return clip['statement'][idx%3]

def violin_pad_collate(batch):
    ...
    clip_ids, vid_feat, sub_input, state_input, modality_text, modality_video = [[x[i] for x in batch] for i in range(6)]
    padded_vid_feat = pad_video_seq(vid_feat) if type(vid_feat[0]) != type(None) else None
    modality_text = torch.Tensor(modality_text).cuda()
    modality_video = torch.Tensor(modality_video).cuda()
    # answer = torch.LongTensor(answer).cuda()

    return clip_ids, padded_vid_feat, sub_input, state_input, modality_text, modality_video
